...
export default function AdminAdvicePage() {
  const [items, setItems] = useState<AdviceInquiry[]>([]);
    ...
    <div className="max-w-6xl mx-auto px-4 py-16">
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 13, color: "var(--text-2)", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
          Community members seeking guidance — {items.length} total
        </p>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
            <thead>
              <tr style={{ background: "var(--surface-2)", borderBottom: "1px solid var(--border)" }}>
                {["Date", "Name", "Email", "Location", "Topic", "Status", "Query"].map((h) => (
              ...
              {items.map((item) => (
                <>
                  ...
                  >
                    ...
                    <td style={{ padding: "11px 14px", fontSize: 12, color: "var(--text-2)", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>{item.location || "—"}</td>
                    <td style={{ padding: "11px 14px", fontSize: 12, color: "var(--text-2)", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>{item.topic || "—"}</td>
                    ...
                    <td style={{ padding: "11px 14px", fontSize: 12, color: "var(--text-2)", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", maxWidth: 200 }}>
                  ...
                  {expanded === item.id && (
                    <tr key={`${item.id}-exp`} style={{ borderBottom: "2px solid rgba(201,169,110,0.22)" }}>
                      <td colSpan={7} style={{ padding: "16px 20px", background: "rgba(201,169,110,0.05)" }}>
                        <div style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.7, fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", whiteSpace: "pre-wrap" }}>
                          <strong style={{ color: "var(--text)" }}>Full Query:</strong><br />{item.query}
                        </div>
                        {item.phone && (
                          <div style={{ marginTop: 8, fontSize: 12, color: "var(--text-2)", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
                            <strong style={{ color: "var(--text)" }}>Phone:</strong> {item.phone}
                          </div>
                        )}
                        <div style={{ marginTop: 12 }}>
                          <a href={`mailto:${item.email}?subject=Re: Your Indiaspora Enquiry`} className="btn btn-sm" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11 }}>
                            Reply via Email
                          </a>
                        </div>
                      </td>
