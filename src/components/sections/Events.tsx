      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between mb-16 gap-6">
          <div className="reveal">
            ...
            <h2 className="text-4xl lg:text-5xl font-bold text-white">Upcoming <span className="gradient-text">Events</span></h2>
            <p className="text-slate-400 mt-3 max-w-lg">Festivals, networking nights, cultural shows, food fairs — never miss what&apos;s happening in the Swiss Indian community</p>
          </div>
            ...
            <div key={event.title} className="reveal glass rounded-2xl overflow-hidden card-hover group border border-white/5 hover:border-white/15 cursor-pointer">
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-500">
                  <MapPin className="w-3.5 h-3.5 text-orange-500 shrink-0" /><span className="truncate">{event.location}</span>
                </div>
                <div className="mt-5 pt-4 border-t border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm text-slate-500"><Calendar className="w-3.5 h-3.5" /><span>{event.date}</span></div>
                  <button className="text-sm font-medium text-orange-400 hover:text-orange-300 transition-colors flex items-center gap-1">Details <ArrowRight className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-12 reveal">
          <div className="glass rounded-2xl p-8 text-center border border-orange-500/20">
            ...
            <button className="px-8 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold hover:from-orange-600 hover:to-amber-600 transition-all shadow-lg shadow-orange-500/25">Submit an Event</button>
          </div>
        </div>
      </div>
    </section>
